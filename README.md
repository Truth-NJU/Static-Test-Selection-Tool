...
3. starts:impacted 显示所有受变更影响的类型(不仅仅是测试类)，从而为变更影响分析提供了一种方法。（注意：更改的类型不包括新出现的或者删除的类型，因此依赖于新出现的类型的测试不会显示出来。且删除的或者新出现的类型也不会显示）
4. starts:select 显示(但不运行)自上次STARTS运行以来受更改影响的测试类，允许开发人员更灵活地首先选择受影响的测试，然后在稍后运行这些测试。（注意：更改的类型不包括新出现的或者删除的类型，因此依赖于新出现的类型的测试不会显示）
5. starts:starts 运行受影响的测试（注意：更改的类型不包括新出现的或者删除的类型，因此依赖于新出现的类型的测试不会被运行）
